...
export function InterviewView({
  ...
  return (
    <div className="max-w-3xl mx-auto w-full space-y-6 animate-in fade-in duration-300 py-4">
      {/* Top Header: Progress & Category Bar */}
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs text-slate-400">
          ...
          <div className="flex items-center gap-2.5">
            <Badge variant="outline" className="text-xs text-slate-300 border-white/[0.08] bg-[#090D14] font-normal">
              {categoryLabels[currentQ.category] || currentQ.category}
            </Badge>
            <span className="font-mono text-slate-400 font-medium">{progressPercent}%</span>
          </div>
        </div>
        ...
        <Progress value={progressPercent} className="h-1.5 bg-slate-800/80" />
      </div>

      {/* Polish Mode Banner */}
      {isElitePolish && (
        <div className="p-4 rounded-xl bg-blue-950/20 border border-blue-500/20 text-xs text-blue-200 flex items-start gap-2.5 shadow-sm">
      ...
      {/* Skip Round 2 Banner if applicable */}
      {roundNumber >= 2 && onSkipToFacts && (
        <div className="p-4 rounded-xl bg-[#090D14]/80 border border-white/[0.08] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-300 shadow-sm">
          <div className="flex items-center gap-2">
      ...
      {/* Central Zen Mode Question Workspace Card */}
      <div className="rounded-2xl border border-white/[0.08] bg-[#0F1623]/90 p-6 sm:p-8 space-y-6 shadow-2xl backdrop-blur-sm">
        {/* Experience Context Badge & Deepening Indicator */}
        <div className="flex flex-wrap items-center gap-2">
          {roundNumber >= 2 && !isElitePolish && (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-0.5 rounded-full bg-blue-500/15 text-blue-300 border border-blue-500/30">
          ...
          {currentQ.relatedExperience && (
            <Badge variant="secondary" className="text-xs text-slate-300 border-white/[0.08] bg-[#090D14] gap-1 font-normal">
        ...
        <div className="space-y-2.5 pt-1">
          ...
          {currentAnswer.value === 'Não se aplica ao meu contexto' && (
            <div className="p-3 rounded-xl bg-[#090D14] border border-white/[0.08] text-slate-300 text-xs flex items-center gap-2 animate-in fade-in">
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl border text-xs sm:text-sm transition-all cursor-pointer ${
                    currentAnswer.value === opt
                      ...
                      : 'border-white/[0.08] bg-[#090D14]/70 text-slate-300 hover:border-slate-700'
                  }`}
                >
                  {opt}
                </button>
        ...
        {/* Well-Spaced Bottom Navigation Bar */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-5 border-t border-white/[0.08]">
          <div className="flex flex-wrap items-center gap-2">
            <Button
              type="button"
              variant="ghost"
            ...
            <Button
              ...
              onClick={handleSkip}
              disabled={isLoading}
              className="text-slate-400 hover:text-slate-200 border-white/[0.08] bg-[#090D14]/50 hover:bg-[#090D14] text-xs h-9 px-3 cursor-pointer"
          ...
          <div>
            {isLastQuestion ? (
              <Button
                ...
                className="w-full sm:w-auto font-medium text-xs sm:text-sm bg-blue-600 hover:bg-blue-500 text-white h-10 px-5 rounded-xl shadow-sm cursor-pointer"
              >
                {isLoading ? (
                  <div className="flex items-center gap-2">
